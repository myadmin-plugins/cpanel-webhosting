...
if (count($_SERVER['argv']) < 3)
	die("Call like {$_SERVER['argv'][0]} <hostname> <domain>\n where <hostname> is a webhosting server such as webhosting2004.interserver.net\n and <domain> is any fqdn that you suspect has adns entry on this server");
...
switch ($db->Record['website_type']) {
	...
	default:
		try {
			...
			$response = json_decode($whm->resetzone($domain));
			print_r($response);
		} catch (Exception $e) {
